          ...
          if_
            ...
            [
              (let digit = uresize (data_in -:. Char.to_int '0') digit_bits in
               let new_joltages =
                 List.map (pairwise (gnd :: joltage_list)) ~f:(fun (prev, curr) ->
            ...
            [
              part1 <-- part1.value +: List.nth_exn joltage_list (part1_digits - 1);
              part2 <-- part2.value +: List.nth_exn joltage_list (part2_digits - 1);
