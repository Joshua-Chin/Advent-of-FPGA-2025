...
let create (scope: Scope.t) ({clock; clear; data_in; data_in_valid}: _ I.t): _ O.t
    ...
    (* Registers for logic *)
    let%hw_var dial =
        let clear_to_50 = Reg_spec.override ~clear_to:(of_int 50 ~width:dial_bits) spec in
    ...
    compile [
        when_ data_in_valid [
            ...
            when_ ((data_in >=:. Char.to_int '0' ) &: (data_in <=:. Char.to_int '9')) [
                hundreds <-- uresize (hundreds.value *: (of_int 10 ~width:4)) output_bits
            ...
            when_ (data_in ==:. Char.to_int '\n') [
                let rotation = uresize (tens.value *: (of_int 10 ~width:4)) dial_bits
                  +: uresize ones.value dial_bits in
